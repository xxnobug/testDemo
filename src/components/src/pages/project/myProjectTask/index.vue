		...
		<el-row >
			<el-col :span="24" class="taskBox" @contextmenu.prevent.native="taskMenu($event)">
				<div class="taskDataBox">
					<div class="taskName">
						<div v-if="!taskNumberEditer" >
							任务编号：{{taskData.name}}
						</div> 
					</div>
					<div class="taskName">
						<div v-if="!taskNumberEditer" >
							项目：{{taskData.project_name}}
						</div> 
					</div>
					<div class="taskTxt">
			...
			uploadLocalDifferencesRes(isSubmit,isSelected) {
				...
				var startUploadRes = (fileListData)=>{
					...
					for(var i=0;i<resData.length;i++){
						...
						var currentCompany = resData[i].company_id
						if(currentCompany==this.$store.state.userInfo.companyid&&resData[i].state!="2"){
							// console.log(1)
							var thArr = {
